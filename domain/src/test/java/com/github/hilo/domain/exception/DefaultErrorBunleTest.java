...
public class DefaultErrorBunleTest {
	...
	@Test public void testGetErrorMessageinteraction() {
		...
		// 3. 见名知意
		when(defaultErrorMessage == null || defaultErrorMessage == "").thenThrow(
						new RuntimeException("defaultErrorMessage is null"));
		// 4. 第一次调用doNothing, 第二次doThrow抛异常
		doNothing().doThrow(new RuntimeException("test void method exception"))
							 .when(mockException)
							 .printStackTrace();
	}
}
