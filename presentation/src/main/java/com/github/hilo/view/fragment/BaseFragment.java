...
public abstract class BaseFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        /**
         * 避免同一个activity有多个fragment切换时，多次调用onCreateView，将rootView做成成员变量实现缓存rootView处理，还要判断当前是否有父布局，做些逻辑处理，不然会报错；
         */
    ...
    /**
     * 恢复数据是在这里处理的，onActivityCreated方法的回调是基于Activity onCreate回调执行完才会执行此方法， 那么当app 切到桌面时，实际上并没有调用onDestroyView，所以下次回到app时，并不会触发该方法
    ...
    private static Callbacks sDummyCallbacks = new Callbacks() {
        @Override
        public void onFragmentItemSelectedCallback(int position, String text) {

        }

        @Override
        public void controlFabBehaviorCallback(boolean scrollingDown) {
