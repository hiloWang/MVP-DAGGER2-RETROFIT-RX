package com.github.hilo.widget.circlebotton;
...
import com.github.hilo.widget.DepthLayout;
import com.github.hilo.widget.interpolator.BackOut;
import com.github.hilo.widget.interpolator.CircInOut;
import com.github.hilo.widget.interpolator.ExpoIn;
import com.github.hilo.widget.interpolator.ExpoOut;
import com.github.hilo.widget.interpolator.QuadInOut;
import com.github.hilo.widget.interpolator.QuintInOut;
...
public class TransitionHelper {
	...
	static ObjectAnimator introAnimate(final DepthLayout target,final float moveY,final float customElevation,long delay,
					int subtractDelay) {

		target.setPivotY(getDistanceToCenter(target));
		target.setPivotX(getDistanceToCenterX(target));
		// 点击fab，要切换的目标 Z轴颜色，可以注释来测试效果
		...
		ObjectAnimator translationX2 = ObjectAnimator.ofFloat(target,View.TRANSLATION_X,
																													-target.getResources().getDisplayMetrics().widthPixels,0)
																								 .setDuration(800);
	...
	static ValueAnimator exitAnimate(final DepthLayout target,final float moveY,final float customElevation,long delay,
		...
		ObjectAnimator elevation = ObjectAnimator.ofFloat(target,"CustomShadowElevation",target.getCustomShadowElevation(),
																											customElevation * target.getResources().getDisplayMetrics().density)
	...
	private static void continueOutToRight(DepthLayout target,float moveY,int subtractDelay) {
		...
		ObjectAnimator translationX2 = ObjectAnimator.ofFloat(target,View.TRANSLATION_X,target.getTranslationX(),
																													target.getResources().getDisplayMetrics().widthPixels)
																								 .setDuration(900);
	...
	@NonNull private static AnimatorListenerAdapter getShowStatusBarListener(final DepthLayout target) {
