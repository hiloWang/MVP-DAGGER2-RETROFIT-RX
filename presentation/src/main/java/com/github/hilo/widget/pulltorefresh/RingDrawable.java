...
class RingDrawable extends RefreshDrawable {
	...
	@Override public void setPercent(float percent) {
		mPaint.setColor(evaluate(percent,mColorSchemeColors[0],mColorSchemeColors[1]));
		mAngle = 340 * percent;
	}

	@Override public void setColorSchemeColors(int[] colorSchemeColors) {
	...
	@Override protected void onBoundsChange(Rect bounds) {
		...
		mHeight = mWidth;
		mBounds = new RectF(bounds.width() / 2 - mWidth / 2,bounds.top,bounds.width() / 2 + mWidth / 2,
												bounds.top + mHeight);
