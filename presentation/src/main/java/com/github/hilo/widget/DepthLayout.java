...
public class DepthLayout extends RelativeLayout {
	...
	class CustomShadow {
		...
		public boolean calculateBounds(DepthLayout target) {
			...
			float[] dst = new float[] {-padding,-padding,target.getWidth() + padding,-padding,-padding,target.getHeight() + padding,target
							.getWidth() + padding,target.getHeight() + padding};
			Matrix matrix = getMatrix();
			matrix.mapPoints(src,dst);
		...
		public void drawShadow(Canvas canvas,DepthLayout dl,Drawable shadow) {
			...
			float[] dst = new float[] {topLeftBack.x,topLeftBack.y,topRightBack.x,topRightBack.y,bottomRightBack.x,
